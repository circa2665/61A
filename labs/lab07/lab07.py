...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        #print('considering', s, 'and prev is', prev)
        if not s:
            return [[]]
        elif s[0] < prev:
            return []
...
def make_glookup(class_assignments):
    ...
    "*** YOUR CODE HERE ***"
    my_grades = {}
    def glookup(name, grade):
        nonlocal my_grades
        my_grades[name] = grade
        my_assignments = my_grades.keys()
        my_points = sum([my_grades[n] for n in my_assignments])
        total_points = sum([class_assignments[n] for n in class_assignments.keys() if n in my_assignments])
        return my_points/total_points
