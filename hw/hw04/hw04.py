...
def balanced(m):
    ...
    if is_planet(m):
        return True
    object_left = end(left(m))
    object_right = end(right(m))
    torque_left = length(left(m))* total_weight(object_left)
    torque_right = length(right(m)) * total_weight(object_right)
    return balanced(object_left) and balanced(object_right) and torque_left == torque_right
